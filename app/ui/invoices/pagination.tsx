...
export default function Pagination({ totalPages }: { totalPages: number }) {
  ...
  const createPageURL = function(pageNumber: number | string): string {
    const pageParams = new URLSearchParams(searchParams);
    const pageNumberInt = parseInt(pageNumber as string)
    if(pageNumberInt > 1){
      pageParams.set('page', pageNumberInt.toString());
    } else {
      pageParams.delete('page');
    }
    
    return pageParams.size > 0 ? `${pathname}?${pageParams.toString()}`: pathname;
  };
